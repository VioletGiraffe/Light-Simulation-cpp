...
void WaveField::resetSimulation()
{
	...
	for (size_t i = 0; i < width; i++)
	{
		for (size_t j = 0; j < height; j++)
		{
			...
			if (sqr(i - width / 2) + sqr(j - height / 2) < sqr(circleRadius))
...
void WaveField::update(const float dt)
{
	...
	if (_time < 2.0f)
	{
		for (int j = height / 2 - 50; j < height / 2; j++) {
			_waveHeight[165][j - 50].fill(std::sin(_time * 100.0f * 0.8f) * 12.0f);
		}
	}
...
std::array<uint8_t, 3> WaveField::pixelColor(size_t x, size_t y) const
{
	...
	for (int k = 0; k < 3; k++)
	{
		size_t colorValue = static_cast<size_t>(sqr(std::min(_accumulatedLight[x][y][k], 1.0f)) * 255.0f);
		if (_pixelMass[x][y] < 1.0f)
